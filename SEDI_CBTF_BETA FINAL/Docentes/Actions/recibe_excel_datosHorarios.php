...
try {
    ...
    $abreviaturas_asignaturas = [
        ...
        'CTSV' => 'CIENCIA, TECNOLOGÍA, SOCIEDAD Y VALORES'
    ];

    $tutorias = [
        'TUTORÍAS I', 'TUTORÍAS II', 'TUTORÍAS III', 
        'TUTORÍAS IV', 'TUTORÍAS V', 'TUTORÍAS VI',
        'TUTORIAS I', 'TUTORIAS II', 'TUTORIAS III', 
        'TUTORIAS IV', 'TUTORIAS V', 'TUTORIAS VI'
    ];

    // Mapa de números romanos a id_semestre
    $mapa_semestres = array(
        'I' => 1,
        'II' => 2,
        'III' => 3,
        'IV' => 4,
    ...
    // Función para dividir nombres complejos
    function dividirNombreCompleto($nombreCompleto) {
        $partes = explode(" ", $nombreCompleto);
        $numPartes = count($partes);

        // Asignación dependiendo de la cantidad de partes
        if ($numPartes >= 4) {
            // Cuando tiene 4 o más partes (caso con nombres compuestos y apellidos compuestos)
            $apellido_paterno = $partes[0];
            $apellido_materno = implode(" ", array_slice($partes, 1, 2)); // Unir las dos siguientes partes
            $nombres = implode(" ", array_slice($partes, 3)); // El resto es el nombre
        } elseif ($numPartes == 3) {
            // Cuando tiene 3 partes (un apellido compuesto)
            $apellido_paterno = $partes[0];
            $apellido_materno = $partes[1];
            $nombres = $partes[2];
        } else {
            // Caso estándar con 2 partes
            $apellido_paterno = $partes[0];
            $apellido_materno = $partes[1];
            $nombres = implode(" ", array_slice($partes, 2)); // El resto es el nombre
        }
    ...
    foreach ($lines as $line) {
        ...
        if (count($clasesPartes) < 3) {
            if(count($clasesPartes)==2){
                array_push($clasesPartes, "BG");
            }else{
        ...
        if (array_key_exists($semestre,$mapa_semestres)) {
            ...
            throw new Exception("El semestre '$id_semestre' no es válido.");
        }

        // Procesar los nombres con la nueva función
        list($apellido_paterno, $apellido_materno, $nombres) = dividirNombreCompleto($profesor);
        ...
        if (in_array($especialidad, ['TÉCNICO EN ADMINISTRACIÓN DE RECURSOS HUMANOS', 'TÉCNICO EN PROGRAMACIÓN', 'BACHILLERATO TECNOLÓGICO'])) {
        ...
        if (!$id_grupo) {
            throw new Exception("El grupo '$nombre_grupo' con semestre '$id_semestre' y especialidad '$id_especialidad' no existe en la base de datos.");
        }
