...
function getPlantilla($folio,
    ...
    $asignaturas = []; // Array para almacenar asignaturas y su conteo de rowspan.

    foreach ($array as $row) {
        $asignaturaCompleta = $row['submodulos'] ? "{$row['asignatura']} - {$row['submodulos']}" : $row['asignatura'];
    
        // Verificar si la asignatura es la misma que la anterior
        if ($asignaturaCompleta == $prevAsignatura) {
            $asignaturas[$asignaturaCompleta]['rowspan']++; // Aumenta el rowspan si es la misma asignatura
        } else {
            $prevAsignatura = $asignaturaCompleta; // Actualiza la asignatura anterior
            $asignaturas[$asignaturaCompleta] = [
                'rowspan' => 1, // Inicializa rowspan a 1 para la nueva asignatura
                'data' => $row // Guarda la fila completa para luego usarla
            ];
        }
    }
    
    // // Generar las filas con rowspan
    ...
    foreach ($array as $index => $row) {
        ...
        // Solo imprimir la celda de asignatura con rowspan la primera vez que aparece
        if ($asignaturas[$asignaturaCompleta]['rowspan'] > 0) {
            $contenido .= "<td class='negritas' style='text-align: center; padding: 2px 2px;' rowspan='{$asignaturas[$asignaturaCompleta]['rowspan']}'>{$asignaturaCompleta}</td>";
            
            // Reducir el rowspan a 0 para no volver a imprimir en las siguientes filas de la misma asignatura
            $asignaturas[$asignaturaCompleta]['rowspan'] = 0;
        }
        
        // Imprimir los demás datos normalmente
