...
<head>
    ...
    <title>Activar Recuperación de Contraseña</title>
...
<body>
    <div>
        <h1>Activar recuperación de contraseña</h1>
        <p>Selecciona la cuenta a la que desea permitir cambiar su contraseña.</p>
    </div>

    <div>
        <table class="display" class="table-style-1" id="tablaUsuariosDocentes" style="width:100%" >
            <thead class="alinear-centro">
                <tr>
                    ...
                    <th scope="col">Recuperar contraseña</th>
                </tr>
            </thead>
            <tbody>
                <?php
                    ...
                    $sqlUsuariosDocentes = "SELECT docentes.id_docente,
                                                    usuarios.id_usuario,
                    ...
                    $queryTableUsuariosDocentes = $conexion->prepare($sqlUsuariosDocentes);
                    $queryTableUsuariosDocentes->execute();
                    
                    foreach ($queryTableUsuariosDocentes as $row) {
                        ...
                        echo "<tr>
                                ...
                                <td class='alinear-centro'>{$row['nombre_docente']}</td>
                                <td class='alinear-izquierda'>{$row['email']}</td>
                                <td class='alinear-centro'>
                                    <button class='edit-btn' data-id='{$row['id_usuario']}' data-name='{$nombreCompleto}'>REESTABLECER</button>
                                    <!-- <button class='info-btn' data-id='{$row['id_docente']}' data-name='{$nombreCompleto}'>Info</button> -->
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#tablaUsuariosDocentes').DataTable({
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                if (confirm('¿Está seguro de que desea REINICIAR LA CONTRASEÑA del usuario: ' + name + '?')) {
                    // Aquí puedes agregar la lógica para editar el alumno
                    window.location.href = 'reiniciar_password.php?id_usuario=' + id; // Aquí puedes redirigir a otra página o abrir un modal para editar el alumno
