        ...
        <table class="display" class="table-style-1" id="tablaAlumnos" style="width:100%" >
            ...
            <tbody>
                <?php
                    ...
                    $sqlUsuarios = "SELECT alumnos.id_alumno,
                                            alumnos.matricula,
                                        ...
                                        FROM 
                                            alumnos
                                        INNER JOIN 
                                            usuarios ON alumnos.id_usuario = usuarios.id_usuario
                                        INNER JOIN 
                                            especialidad ON alumnos.id_especialidad = especialidad.id_especialidad
                                        INNER JOIN 
                                            grupo ON alumnos.id_grupo = grupo.id_grupo
                                        ORDER BY 
                                            alumnos.apellido_paterno ASC;";
                    $queryTableUsuariosAlumnos = $conexion->prepare($sqlUsuarios);
                    $queryTableUsuariosAlumnos->execute();
                    
                    foreach ($queryTableUsuariosAlumnos as $row) {
                        ...
                        echo "<tr>
                                ...
                                <td class='alinear-centro'>{$row['usuario']}</td>
                                <td class='alinear-centro'>{$row['password']}</td>
