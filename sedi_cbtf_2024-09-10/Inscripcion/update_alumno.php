...
if($ticket == 1 && $status == 'PENDIENTE'){
    ...
    $espeSel = "SELECT id_especialidad, nombre_grupo FROM grupo WHERE id_grupo = :id_grupo;";
    $especSelect = $conexion->prepare($espeSel);
    $especSelect->bindParam(':id_grupo', $id_grupo, PDO::PARAM_STR);
    $especSelect->execute();
    ...
    $grupoSel = "SELECT id_grupo FROM grupo	WHERE nombre_grupo = :nombre_grupo AND id_especialidad = :id_especialidad AND id_semestre = :id_semestre;";
    $grupoSelect = $conexion->prepare($grupoSel);
    $grupoSelect->bindParam(':nombre_grupo', $nombre_grupo, PDO::PARAM_STR);
    $grupoSelect->bindParam(':id_especialidad', $id_especialidad, PDO::PARAM_STR);
    $grupoSelect->bindParam(':id_semestre', $id_semestre, PDO::PARAM_STR);
    $grupoSelect->execute();
    
    $resulta = $grupoSelect->fetch(PDO::FETCH_ASSOC);
    
    if ($resulta) {
        $id_grupo = $resulta['id_grupo'];
    } else {
        // Manejar el caso donde no se encontró la especialidad
        echo "No se encontró el grupo";
        exit;   
