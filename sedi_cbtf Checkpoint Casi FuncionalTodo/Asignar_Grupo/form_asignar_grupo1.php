    ...
    <div>
        ...
        <form method="post" action="update_grupo.php">
            <label for="nuevo_grupo">Seleccionar Grupo:</label>
                <select class="form-control" name="nuevo_grupo" id="nuevo_grupo">
                    <?php 
            ...
            <tbody>
                <?php
                    $sqlAlumnos = "SELECT a.id_alumno,
                                    ...
                                    JOIN
                                        especialidad e ON g.id_especialidad = e.id_especialidad
                                    WHERE (g.id_semestre = '1') AND a.status = 'INSCRITO' 
...
?>
        ...
        <button class="enlace-verde-grande2" type="submit" onclick="return confirm('¿Estás seguro de que deseas asignar estos alumnos a este grupo?');">Actualizar Grupo</button>
        </form>
    </div>
            ...
            if (id_grupo) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#conteoGenero').html(
                        data); // Actualiza el contenido de conteoGenero con el resultado
                    }
                });
            } else {
