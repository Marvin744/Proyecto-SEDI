...
<body>
    ...
    <h1>Horario Actividades Complementarias</h1>

    <form method="post">
        <button type="submit" class="btn-azul" name="insert_activity">Insertar Nueva Actividad</button>
    </form>
    ...
    <table class="f19_table">
        ...
        <tfoot>
            ...
            <tr class="f19_header-row">
                ...
                <td class="f19_td" id="subtotal1-lunes-total">0</td>
                <td class="f19_td" id="subtotal1-martes-total">0</td>
                <td class="f19_td" id="subtotal1-miercoles-total">0</td>
                <td class="f19_td" id="subtotal1-jueves-total">0</td>
                <td class="f19_td" id="subtotal1-viernes-total">0</td>
                <td class="f19_td" id="subtotal1-sabado-total">0</td>
                <td class="f19_td" id="subtotal1-horas-total"><strong><?php echo $totalHorasSemanales; ?></strong></td>
            </tr>
        </tfoot>
    </table>
    <br>

    <button id="verificar-horas" class="btn-verde">Verificar Horas</button>
    ...
    <div class="datos_f19">
        <form action="plantilla_f19.php" method="post" target="_blank">
            ...
            <button type="submit" class="btn-rojo">Exportar a PDF</button>
        </form>
    </div>
    <script>
        ...
        document.querySelector('form[action="plantilla_f19.php"]').addEventListener('submit', function() {
            ...
            document.getElementById('hiddenSubtotalMartes1').value = document.getElementById('subtotal1-martes')
                .textContent;
            document.getElementById('hiddenSubtotalMiercoles1').value = document.getElementById('subtotal1-miercoles')
                .textContent;
            ...
            document.getElementById('hiddenSubtotalHoras').value = document.getElementById('subtotal1-horas')
                .textContent;
            // document.getElementById('hiddenSubtotalHoras').value = document.getElementById('subtotal-horas1')
            //     .textContent;
        });
...
function updateSubtotalsAct() {
    ...
    days.forEach(day => {
        const dayCells = document.querySelectorAll(`table:nth-of-type(2) td[data-day1="${day}"]`);
        let subtotal = 0;
...
function updateTotalHorasDias() {
    ...
    days.forEach(day => {
        ...
        document.querySelectorAll(`table:nth-of-type(1) td[data-day="${day}"]`).forEach(cell => {
            ...
            if (cellText) {
                ...
                ranges.forEach(range => {
                    ...
                    if (times.length === 2) {
                        ...
                        const end = times[1].split(':').map(Number);
                        const duration = (end[0] * 60 + end[1]) - (start[0] * 60 + start[1]);
                        totalHorasTabla1 += duration / 60; // Convertimos minutos a horas
                    }
                });
            }
        });
        ...
        document.querySelectorAll(`table:nth-of-type(2) td[data-day1="${day}"]`).forEach(cell => {
            const cellText = cell.textContent.trim();
            if (cellText) {
                const ranges = cellText.split(' ');
                ranges.forEach(range => {
                    ...
                    if (times.length === 2) {
                        ...
                        const end = times[1].split(':').map(Number);
                        const duration = (end[0] * 60 + end[1]) - (start[0] * 60 + start[1]);
                        totalHorasTabla2 += duration / 60; // Convertimos minutos a horas
                    }
                });
            }
        });
        ...
        document.getElementById(`subtotal1-${day}-total`).textContent = totalHorasDia; // Mostrar el total como entero
    });
...
function updateTotalHorasSemanal() {
    ...
    document.getElementById('subtotal1-horas-total').textContent = totalHorasSemanales; // Mostrar el total como entero
}
